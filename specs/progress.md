...
### 任务 3.3.1-3.3.4：视频导出和下载

**创建的文件**：

| 文件                         | 作用          |
| ---------------------------- | ------------- |
| `api/export.py`              | 导出 API 路由 |
| `components/ExportPanel.jsx` | 导出面板组件  |
| `components/ExportPanel.css` | 组件样式      |

**新增 API**：

| 接口                                 | 说明           |
| ------------------------------------ | -------------- |
| `GET /api/export/download/{task_id}` | 下载单个视频   |
| `POST /api/export/package`           | 批量打包 (ZIP) |
| `GET /api/export/collection`         | 获取已完成列表 |
| `GET /api/export/zip/{filename}`     | 下载 ZIP       |
